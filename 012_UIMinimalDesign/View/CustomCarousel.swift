...
struct CustomCarousel<Content: View, Item, ID>: View where Item: RandomAccessCollection, ID: Hashable, Item.Element: Equatable {
	...
	init(
		...
		self.cardPaddding = cardPaddding
		self.items = items
	}
	...
	var body: some View {
		GeometryReader { proxy in
			let size = proxy.size
			let cardWidth = size.width - (cardPaddding - spacing)
			
			HStack(spacing: spacing) { 
				ForEach(items, id: id) { movie in
					let index = indexOf(item: movie)
					
					content(movie, CGSize(width: size.width - cardPaddding, height: size.height))
						.frame(width: size.width - cardPaddding, height: size.height)
	...
	func onEnd(value: DragGesture.Value, cardWidth: CGFloat) {
		...
		withAnimation(.interactiveSpring(response: 0.4, dampingFraction: 1, blendDuration: 1)) {
			...
			let extraSpace = index == 0 ? 0 : (cardPaddding / 2)
